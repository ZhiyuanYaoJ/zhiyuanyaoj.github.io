...
Another interesting point is that in **VPP**, when using _GRE_, the sender (in our case `vpp1`) always drop the packets silently while the receiver could en/decapsulate the packets normally. This remains another task to work on in the future, that is, trying to play with en/decapsulation as well as routing and forwarding packets to some `namespace` out of **VPP**.
